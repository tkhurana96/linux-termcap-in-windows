...
namespace emlcw {
	...
	private:
		HANDLE _h;
		bool greaterThanWindows10;
		CONSOLE_SCREEN_BUFFER_INFO buffInfo;

		void emulate_clear_screen() {
			if (greaterThanWindows10) {
	...
	public:
		console() {
			...
			if (_h != INVALID_HANDLE_VALUE) {
				greaterThanWindows10 = IsWindows10OrGreater();
				if (greaterThanWindows10) {
					DWORD dwMode = 0;
					GetConsoleMode(_h, &dwMode);
					dwMode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING;
					SetConsoleMode(_h, dwMode);
				}
			}
		}
		...
		int execute_cap(emlcw::numeric cap) {
			switch (cap) {
			case emlcw::numeric::columns: return getCols();
			}
		}

		void execute_cap(emlcw::str cap, valType p1=0, valType p2=0, valType p3=0, valType p4=0, valType p5 = 0,
										valType p6 = 0, valType p7 = 0, valType p8 = 0, valType p9 = 0, valType p10 = 0,
										valType p11 = 0, valType p12 = 0, valType p13 = 0, valType p14 = 0, valType p15 = 0,
										valType p16 = 0) {
			switch (cap) {
