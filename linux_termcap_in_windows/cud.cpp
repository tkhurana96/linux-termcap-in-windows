...
auto someCursorMovingUpTask() {
	auto consoleHandle = GetStdHandle(STD_OUTPUT_HANDLE);
	CONSOLE_SCREEN_BUFFER_INFO buffInfo;
	GetConsoleScreenBufferInfo(consoleHandle, &buffInfo);
	SetConsoleCursorPosition(consoleHandle, { buffInfo.dwCursorPosition.X, buffInfo.dwCursorPosition.Y - 8 });
}

auto emulate_cud(const short jump = 1) {
	if (jump > 0) {
		auto consoleHandle = GetStdHandle(STD_OUTPUT_HANDLE);
		CONSOLE_SCREEN_BUFFER_INFO buffInfo;
		GetConsoleScreenBufferInfo(consoleHandle, &buffInfo);
...
int main() {
	...
	std::cout << s;
	someCursorMovingUpTask();
	char bogus;

	std::cin >> bogus;
	emulate_cud(20);
	std::cout << "THIS LINE IS OVERWRITTEN";
